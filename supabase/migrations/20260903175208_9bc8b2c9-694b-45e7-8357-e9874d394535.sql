...
create table public.test_entries (
  ...
  user_id uuid not null references auth.users(id) on delete cascade,
  date date not null,
  mcv numeric, hb numeric, iron numeric, tibc numeric, ferritin numeric,
  tsh numeric, reticulocytes numeric, retic_index numeric,
  bilirubin_indirect numeric, creatinine numeric, b12 numeric,
  folate numeric, epo numeric,
  morphology text,
  notes text,
  created_at timestamptz not null default now()
);
